...
/// <summary>
/// 플레이어 쪼그리기 동작, 이동 속도 감소, 애니메이션 및 무적 타이머를 포함한 사망·리스폰 시퀀스 전체를 관리합니다.
/// </summary>
public class PlayerCrouchDeath : MonoBehaviour
{
    ...
    [Tooltip("리스폰 직후 이 시간(초) 동안은 발각/처형 트리거 무시 (리스폰 지점이 위험구역 안일 때 즉시 재사망 방지)")]
    [SerializeField] private float _respawnImmuneDuration = 1.5f;
    public bool IsRespawnImmune => _respawnImmuneTimer > 0f;
    private float _respawnImmuneTimer = 0f;
    ...
    private void Update()
    {
        ...
        if (_respawnImmuneTimer > 0f)
            _respawnImmuneTimer -= Time.deltaTime;
    ...
    public void ToggleCrouch()
    {
        ...
        MagneticManager.Instance?.SetDetectUIVisible(!_isCrouching);
    }
    ...
    public void OnDeath(Vector3? killerPosition = null)
    {
        ...
        // Hold(부착/소지) 상태는 아니지만 IsInteracting만 켜진 채 남는 앵커 정리 (리스폰 후 E 중복 입력 방지)
        MagneticManager.Instance?.CurrentInteractingAnchor?.ForceEndInteractionOnDeath();
        MagneticManager.Instance?.BlockDetectionTemporarily(9999f); // 리스폰 전까지 차단
        MagneticManager.Instance?.ClearInteractionState(); // 사망 시 인터랙션 대상/UI 강제 초기화
    ...
    private IEnumerator RespawnCoroutine()
    {
        ...
        _respawnImmuneTimer = _respawnImmuneDuration; // 리스폰 지점이 위험구역 안이어도 즉시 재발각되지 않도록
        if (_animator != null)
        ...
        // 사망~리스폰 사이 위치에서 텔레포트 전 재진입된 인터랙션 잔여 상태 한 번 더 정리
        // (텔레포트는 OnTriggerExit를 안정적으로 발생시키지 않아 _collisionActiveObjects에 잔류 가능)
        MagneticManager.Instance?.ClearInteractionState();
