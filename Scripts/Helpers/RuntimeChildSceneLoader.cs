...
public class RuntimeChildSceneLoader : MonoBehaviour
{
    ...
    private IEnumerator LoadChildScenesCoroutine()
    {
        ...
        int totalScenes = childSceneNames.Count;
        ...
        foreach (string sceneName in childSceneNames)
        {
            ...
            {
                LoadingProgress = (completedScenes + asyncLoad.progress) / totalScenes;
                yield return null;
            }

            DebugLogger.Log($"씬 '{sceneName}' 로딩 완료!");
            completedScenes++;
            LoadingProgress = (float)completedScenes / totalScenes;
        }
