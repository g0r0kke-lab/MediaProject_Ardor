...
namespace StarterAssets
{
	public class StarterAssetsInputs : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			// UI 모드가 아닐 때만 직접 키보드 입력 처리
			if (InputModeManager.GetCurrentMode() != InputMode.UI)
			{
				bool currentShiftState = Keyboard.current.leftShiftKey.isPressed;
				
				// 쪼그린 상태 또는 일어서는 중이면 sprint 강제 false
				if (_playerCrouchDeath != null && (_playerCrouchDeath.IsCrouching || _playerCrouchDeath.IsStandingUp || _sprintAfterCrouchCooldown > 0f))
				{
					sprint = false;
				}
				else if (currentShiftState != sprint)
				{
					sprint = currentShiftState;
				}
			}
